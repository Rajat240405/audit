...
Everything else (no serial column, PyMuPDF missing, detection failure)
falls back to exactly the previous behaviour — plain stream text / the pypdf
call in ``scraper._extract_text_from_document`` — so unrelated ingestion
output is unchanged. No layout models, no OCR, no network.

License note: PyMuPDF is AGPL-3.0/commercial dual-licensed (already in use
under src/scripts/). Kept as a lazy import so production keeps working
without it.
"""
...
_MAX_PITCH = 32.0
_LINE_MERGE_TOL = 2.0   # pt — same-visual-line baseline wobble (pypdf parity):
...
def extract_pdf_text(data: bytes) -> str | None:
    ...
    that as "scanned"); raises ImportError when PyMuPDF is unavailable so the
    caller can take its legacy pypdf path unchanged.
    """
    fitz = _import_fitz()
...
def _render_merged(lines: list[dict]) -> str:
    ...
    fused with a single space, preserving y,x reading order — the same line
    granularity pypdf's extract_text() produces on these documents."""
