...
def category_run(
    ...
    *,
    ...
    # both v2 listing categories (reports, press-release) are real taxonomy
    # roots upstream, so a missing parent is run-fatal (scope unprovable)
    ...
    for post in posts:
        ...
        family = None
        if cfg_families:
            # reports: only the three approved families pass; anything else
            # (ncaer-reports, account-at-glance, achievements, general-report)
            # is scoped out fail-closed.
