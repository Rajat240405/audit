...
def main(argv: list[str] | None = None, *, transport=None, sleeper=None) -> int:
    ...
    sess_hint = f"sessions={sessions}" if sessions else "sessions=config-range"
    min_hint = f"ministries={[m['slug'] for m in ministries]}"
    print(f"[start] rajya-sabha root={root} {sess_hint} {min_hint}")
    if opts.dry_run:
    ...
    for i, s in enumerate(report["sessions"], 1):
        line = (f"  [{i}/{n_sessions}] session {s['session']}: {s['status']:9s} "
