...
PHASE 5 — CONFIGURATION-DRIVEN TREE
-----------------------------------
...
``resolve_orgs``). If the config is missing/unreadable/unparseable, the
shipped snapshot below is used (same availability idiom as
``src/scripts/ingest.py``'s builtin fallback). A config that is present but
STRUCTURALLY invalid raises ``ValueError`` naming the offending key.
...
# ─────────────────────────────────────────────────────────────────────────────
# Shipped snapshot — used ONLY when config/sources.yaml is unreadable (kept in
# sync with the `presentation:` block; asserted equal by
# tests/test_phase5_config_driven_sources.py). Edit the YAML, not this.
# ─────────────────────────────────────────────────────────────────────────────
...
def _read_yaml(path: Path) -> dict | None:
    ...
    except Exception:  # noqa: BLE001 — unreadable/invalid YAML -> snapshot fallback
        return None
...
def _derive(path: Path) -> tuple[dict, dict[str, str], dict[str, str]]:
    ...
    if data is None:  # missing/unreadable/unparseable -> availability fallback
        return (
            {k: {"name": v["name"], "orgs": [dict(o) for o in v["orgs"]]}
             for k, v in _SHIPPED_TREE.items()},
            dict(_SHIPPED_CATEGORY_LABELS),
            dict(_SHIPPED_DT_CATEGORY),
        )
...
def _load_initial() -> None:
    """Import-safe populate: any failure leaves the shipped snapshot active."""
    global ORG_TREE, _CATEGORY_LABELS, _DT_CATEGORY
    try:
        _ensure_fresh()
    except Exception:  # noqa: BLE001
        ORG_TREE = {k: {"name": v["name"], "orgs": [dict(o) for o in v["orgs"]]}
                    for k, v in _SHIPPED_TREE.items()}
        _CATEGORY_LABELS = dict(_SHIPPED_CATEGORY_LABELS)
        _DT_CATEGORY = dict(_SHIPPED_DT_CATEGORY)
